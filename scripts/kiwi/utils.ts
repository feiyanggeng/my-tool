...
function getProjectConfig() {
  const rootDir = path.resolve(process.cwd(), singleArgvData.getProductDir());
...
function getLangDir(lang) {
  const langsDir = singleArgvData.getProductDir();
...
const singleArgvData =  (() => {
  let productDir = "";
  let setProductDir = (dir: string) => {
    productDir = dir;
  };
  let getProductDir = () => {
    return productDir;
